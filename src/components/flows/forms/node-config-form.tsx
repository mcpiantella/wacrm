...
export function NodeConfigForm({
  ...
  switch (node.node_type) {
    ...
    case "handoff":
      return (
        <TextRow
          label="Nota interna (para o atendente que irá assumir)"
    ...
    case "end":
      return (
        <p className="text-xs text-muted-foreground">
          Nó terminal. Quando o motor chega a este nó, a execução é marcada como
...
function SendButtonsForm({
  ...
  return (
    <>
      ...
      />
      <TextRow
        label="Rodapé (opcional, 60 chars)"
      ...
      <div>
        <div className="mb-2 flex items-center justify-between">
          <label className="text-xs text-muted-foreground">
            Botões (1–3) — cada um direciona para um nó diferente
          </label>
        </div>
          ...
          {buttons.map((b, i) => (
            ...
            >
              ...
              <Input
                ...
                placeholder="Título visível (≤20 chars)"
                className="bg-muted"
                maxLength={20}
              />
              <NodeKeySelect
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <TextRow
          label="Rótulo do botão para expandir (≤20 chars)"
  ...
  const handleFile = useCallback(
    async (file: File) => {
      if (file.size > MEDIA_MAX_BYTES) {
        toast.error(
          `Arquivo com ${(file.size / 1024 / 1024).toFixed(1)} MB — limite é 16 MB.`,
        );
        return;
      }
      setUploading(true);
